...
public class InventarioJugador : MonoBehaviour
{
    ...
    [SerializeField] private GameObject pico;
    private GameObject objetoActual;

    [Header("Inputs")]
    [SerializeField] private KeyCode teclaPanel = KeyCode.P;
    [SerializeField] private KeyCode teclaHacha = KeyCode.Alpha1;
    [SerializeField] private KeyCode teclaMachete = KeyCode.Alpha2;
    [SerializeField] private KeyCode teclaPico = KeyCode.Alpha3;

    [Header("Golpe con herramienta")]
    [SerializeField] private float rangoGolpe = 2f;
    [SerializeField] private LayerMask capaGolpeables;
    [SerializeField] private float cdAtaque = 0.45f;
    private bool puedeAtacar = true;
    private Coroutine swingCR;
    ...
    private Vector3 hachaPos0;
    private Quaternion hachaRot0;
    private Vector3 picoPos0;
    private Quaternion picoRot0;
    ...
    void Start()
    {
        ...
        if (pico != null)
        {
            picoPos0 = pico.transform.localPosition;
            picoRot0 = pico.transform.localRotation;
    ...
    void ManejarInventario()
    {
        ...
        if (Input.GetKeyDown(teclaPico))
            ToggleObjeto(pico);

        if ((objetoActual == hacha || objetoActual == pico) && Input.GetMouseButtonDown(0))
        {
            if (puedeAtacar && swingCR == null)
                swingCR = StartCoroutine(AnimacionAtaque(objetoActual));
        }
    }
    ...
    void EquiparObjeto(GameObject objeto)
    {
        ...
        if (objetoActual != null)
        {
            ...
            else if (objetoActual == pico)
            {
                pico.transform.localPosition = picoPos0;
                pico.transform.localRotation = picoRot0;
            }
            objetoActual.SetActive(true);
        }
    }
    ...
    // ================== ATAQUE ==================
    IEnumerator AnimacionAtaque(GameObject herramienta)
    {
        if (herramienta == null) yield break;

        puedeAtacar = false;
        float duracionGolpe = 0.25f;
        float duracionVuelta = 0.30f;

        Vector3 posInicial = herramienta.transform.localPosition;
        Quaternion rotInicial = herramienta.transform.localRotation;

        Vector3 posAtras = posInicial + new Vector3(0f, 0f, -0.2f);
        Quaternion rotAtras = Quaternion.Euler(rotInicial.eulerAngles + new Vector3(-25f, 0f, 0f));

        Vector3 posAdelante = posInicial + new Vector3(0f, 0f, 0.3f);
        Quaternion rotAdelante = Quaternion.Euler(rotInicial.eulerAngles + new Vector3(35f, 0f, 0f));

        float t = 0f;
        while (t < duracionGolpe * 0.5f)
        {
            herramienta.transform.localPosition = Vector3.Lerp(posInicial, posAtras, t / (duracionGolpe * 0.5f));
            herramienta.transform.localRotation = Quaternion.Slerp(rotInicial, rotAtras, t / (duracionGolpe * 0.5f));
            t += Time.deltaTime;
            yield return null;
        }
        ...
        while (t < duracionGolpe * 0.5f)
        {
            herramienta.transform.localPosition = Vector3.Lerp(posAtras, posAdelante, t / (duracionGolpe * 0.5f));
            herramienta.transform.localRotation = Quaternion.Slerp(rotAtras, rotAdelante, t / (duracionGolpe * 0.5f));
            t += Time.deltaTime;
            yield return null;
        }
        ...
        while (t < duracionVuelta)
        {
            herramienta.transform.localPosition = Vector3.Lerp(posAdelante, posInicial, t / duracionVuelta);
            herramienta.transform.localRotation = Quaternion.Slerp(rotAdelante, rotInicial, t / duracionVuelta);
            t += Time.deltaTime;
            yield return null;
        }

        herramienta.transform.localPosition = posInicial;
        herramienta.transform.localRotation = rotInicial;
    ...
    void DetectarImpacto()
    {
        ...
        Ray rayo = new Ray(cam.transform.position, cam.transform.forward);
        if (Physics.Raycast(rayo, out RaycastHit hit, rangoGolpe, capaGolpeables, QueryTriggerInteraction.Ignore))
        {
    ...
    public void AgregarRecurso(string tipo, int cantidad)
    {
        switch (tipo)
        {
            ...
            case "Piedra":
                ...
                break;
            default:
                Debug.LogWarning("Tipo de recurso no reconocido: " + tipo);
                break;
        }
    ...
    {
        if (troncosTexto != null)
            troncosTexto.text = $"Troncos: {troncos} | Piedras: {piedras}";
    }
}
*/