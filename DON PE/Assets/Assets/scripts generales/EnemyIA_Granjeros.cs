...
public class EnemyIA_Granjeros : MonoBehaviour, IDañoRecibible
{
    ...
    // NUEVO: control de actividad según el sol
    ...
    {
        activo = !esDia; // activo solo de noche
    ...
    {
        if (!estaVivo) return;
        if (!activo) return; // no hace nada durante el día
...
public class EnemyIA_Granjeros : MonoBehaviour, IDañoRecibible
{
    ...
    [Header("Patrulla y rastreo")]
    public float radioMerodeo = 10f;
    public float tiempoEsperaPatrulla = 3f;
    public float radioBusquedaInicial = 50f;
    public float intervaloPistas = 5f;
    private Coroutine rastreoCR;
    ...
    void OnCambioDiaNoche(bool esDia)
    {
        activo = !esDia; // solo activo de noche
        if (agent != null) agent.isStopped = esDia;
    ...
    void BuscarNuevoObjetivo()
    {
        GameObject objetivo = BuscarMasCercanoPorTag(tagTrigo);
        if (objetivo == null)
            objetivo = BuscarMasCercanoPorTag(tagHoguera);
        if (objetivo == null)
            objetivo = BuscarMasCercanoPorTag(tagLobo);

        if (objetivo != null)
        {
            objetivoActual = objetivo;
            estado = Estado.YendoObjetivo;
            if (rastreoCR != null) StopCoroutine(rastreoCR);
        }
        else
        {
            if (rastreoCR == null)
                rastreoCR = StartCoroutine(RastrearObjetivoCR());
        }
    }

    IEnumerator RastrearObjetivoCR()
    {
        estado = Estado.Buscando;
        float tiempo = 0f;

        while (estado == Estado.Buscando && tiempo < 60f)
        {
            Vector3 randomDir = Random.insideUnitSphere * radioBusquedaInicial;
            randomDir.y = 0;
            Vector3 destino = puntoOrigen + randomDir;

            if (NavMesh.SamplePosition(destino, out NavMeshHit hit, radioBusquedaInicial, NavMesh.AllAreas))
                agent.SetDestination(hit.position);

            yield return new WaitForSeconds(intervaloPistas);

            GameObject posible = BuscarMasCercanoPorTag(tagTrigo);
            if (posible != null)
            {
                objetivoActual = posible;
                estado = Estado.YendoObjetivo;
                rastreoCR = null;
                yield break;
            }

            tiempo += intervaloPistas;
        }
