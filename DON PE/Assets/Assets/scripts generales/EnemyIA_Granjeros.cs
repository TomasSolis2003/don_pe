...
public class EnemyIA_Granjeros : MonoBehaviour, IDañoRecibible
{
    ...
    [Header("Jugador")]
    public Transform jugadorManual;          // arrastrá el Player si querés
    ...
    IEnumerator HuirCR()
    {
        ...
        Vector3 dir = Random.insideUnitSphere * 20f; dir.y = 0f;
        Vector3 destino = transform.position + dir;

        if (NavMesh.SamplePosition(destino, out NavMeshHit hit, 10f, NavMesh.AllAreas))
            agent.SetDestination(hit.position);

        yield return new WaitForSeconds(duracionHuida);
        Desaparecer();
    }
